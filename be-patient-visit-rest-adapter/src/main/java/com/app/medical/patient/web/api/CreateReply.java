...
public record CreateReply(
    ...
    public static CreateReply of(final PatientResult patientResult) {
        if (patientResult.succeeded()) {
            return new CreateReply(ReplyCode.CREATED, null);
        }
        return new CreateReply(ReplyCode.NOT_CREATED, List.of(patientResult.patientResultErrorCode().name()));
    }

    public static CreateReply of(final PatientVisitResult patientVisitResult) {
        if (patientVisitResult.succeeded()) {
            return new CreateReply(ReplyCode.CREATED, null);
        }
        return new CreateReply(ReplyCode.NOT_CREATED,
